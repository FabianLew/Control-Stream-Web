...
export function ConnectionGrid() {
  ...
  const { data: connections, isLoading, isError } = useQuery<ConnectionOverviewDto[]>({
  ...
  const handleEditClick = (e: React.MouseEvent, conn: ConnectionOverviewDto) => {
    e.preventDefault();
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-[180px] rounded-xl border bg-card shadow-sm p-6 space-y-4">
        ...
        {connections?.map((conn) => (
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-start justify-between">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={(e) => handleEditClick(e, conn)}>
              ...
              {/* Sekcja Danych Technicznych - Styl z StreamList */}
            ...
            <div className="px-5 py-3 border-t border-border bg-muted/20 flex items-center justify-between rounded-b-xl">
               ...
               <Link href={`/connections/${conn.id}`}>
                 <Button variant="ghost" size="sm" className="h-7 px-2 text-xs gap-1.5 hover:bg-background border border-transparent hover:border-border">
